...
Specify Public API Standards within the Federated Post-Secondary Digital Commons. This outline inherits the
...
- In scope: requirements, interfaces, dependencies, data, security, operations,
  validation, and roadmap decisions for Public API Standards.
...
- Public interfaces use REST/OpenAPI and gRPC/Protobuf where their tradeoffs justify them; artifacts use OCI standards.
- SDKs and tooling wrap versioned platform contracts rather than exposing internal service or database details.
- Define the interface with OpenAPI when REST is selected and document compatibility/versioning behavior.
