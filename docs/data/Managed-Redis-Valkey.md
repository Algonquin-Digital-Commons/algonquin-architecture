...
Specify Managed Redis Valkey within the Federated Post-Secondary Digital Commons. This outline inherits the
...
- In scope: requirements, interfaces, dependencies, data, security, operations,
  validation, and roadmap decisions for Managed Redis Valkey.
...
- PostgreSQL and Redis/Valkey-compatible systems are default reference primitives where their semantics fit.
- Services own their data through documented APIs and migrations; no generic proprietary Algonquin database is created.
- Select an established data engine by workload requirements and expose it through owned service contracts.
