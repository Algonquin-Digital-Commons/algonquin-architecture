...
Specify Desktop Local Tool Sandbox within the Federated Post-Secondary Digital Commons. This outline inherits the
...
- In scope: requirements, interfaces, dependencies, data, security, operations,
  validation, and roadmap decisions for Desktop Local Tool Sandbox.
...
- The Session Host mediates commands, files, network, tools and external actions through explicit workspace grants, action digests, policy, timeouts, resource limits and receipts.
- The OpenWork renderer never owns host or provider credentials and cannot call privileged OS APIs outside reviewed IPC allowlists.
