...
Specify Desktop Architecture within the Federated Post-Secondary Digital Commons. This outline inherits the
...
- In scope: requirements, interfaces, dependencies, data, security, operations,
  validation, and roadmap decisions for Desktop Architecture.
...
- Requirements and quality attributes: cross-platform workspace client, offline-local operation, explicit grants, signed updates, accessibility, and bounded downstream maintenance; measurable targets remain to be set.
- Interfaces, APIs, events, and contracts: Commons AI Gateway plus the shared Agent Session Contract; OpenCode is the first replaceable host adapter.
- Dependencies and ownership boundaries: OpenWork-derived UI/shell owns presentation; Session Host owns execution; policy owns authorization; gateway owns inference; relay owns only encrypted transport.
...
- Standards, upstream projects, and build/adopt/fork decision: ADR-0018 selects an exact-commit, MIT-only OpenWork downstream outside `ee/`; source import remains gated.
...
- OpenWork Den, `ee/`, hosted MCP/inference, provider bypasses, and automatic privileged-action approval are excluded.
