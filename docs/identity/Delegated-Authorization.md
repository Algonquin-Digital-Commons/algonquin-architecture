...
Specify Delegated Authorization within the Federated Post-Secondary Digital Commons. This outline inherits the
...
- In scope: requirements, interfaces, dependencies, data, security, operations,
  validation, and roadmap decisions for Delegated Authorization.
...
- The deployment's institution-approved IdP remains the identity root; no separate Commons password identity is created. Algonquin uses Entra through its deployment adapter.
