...
Specify Federation Policy within the Federated Post-Secondary Digital Commons. This outline inherits the
...
- In scope: requirements, interfaces, dependencies, data, security, operations,
  validation, and roadmap decisions for Federation Policy.
...
- Federation is optional and local operation is complete without peers. The first production mode is an allowlist pilot; each institution owns its domains, peers, blocks, keys, moderation, retention and exit.
...
- [ActivityPub Federation](../architecture/03-activitypub-federation.md)
