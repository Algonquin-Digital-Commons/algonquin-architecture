...
Specify Brightspace API Adapter within the Federated Post-Secondary Digital Commons. This outline inherits the
...
- In scope: requirements, interfaces, dependencies, data, security, operations,
  validation, and roadmap decisions for Brightspace API Adapter.
...
- Brightspace integration uses supported D2L APIs, OAuth, and LTI mechanisms rather than scraping or unofficial protocols.
- Connectors translate external LMS contracts into versioned internal academic contracts with least privilege.
- Clients and agents consume the internal Academic Service; College-approved Brightspace is authoritative for production academic data while development uses synthetic local/test providers.
- Conform to ADR-0007: use supported D2L interfaces with least privilege and no scraping.
