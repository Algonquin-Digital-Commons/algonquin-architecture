# AC AI Platform Architecture

> Status: Accepted baseline; implementation incomplete  
> Domain: vision  
> Owner: AC AI architecture  
...
- Constraint: the MVP works with one local backend; cloud and ACF remain optional
...
Web | OpenCode | Desktop | Mobile | CLI | SDK | academic clients
           ...
           identity | authorization | policy | usage | audit | API contracts
              ...
              model registry + stable aliases + capability router
                                  |
       local runtime | dedicated cluster | ACF adapter | user-owned edge adapter
                                  |
...
- `/commons/v1/...`: Algonquin-native identity, usage, projects, policies, academic,
...
retention, cost and fallback. Placement follows ADR-0013's locality ladder. ACF
...
Algonquin AI Web is implementation-independent at the architecture boundary. Its
...
- Accepted baseline: Python/FastAPI gateway, vLLM production adapter, llama.cpp
  local adapter, PostgreSQL/pgvector, and provider-neutral contracts.
- Remaining work: Select exact model releases and licenses, implement contracts,
...
- [AI Web Foundation](../../clients/Commons-AI-Web-Foundation.md)
