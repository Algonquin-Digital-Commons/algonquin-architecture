...
Specify Worker Agent Specification within the Federated Post-Secondary Digital Commons. This outline inherits the
...
- In scope: requirements, interfaces, dependencies, data, security, operations,
  validation, and roadmap decisions for Worker Agent Specification.
...
- ACF owns campus-specific enrollment, topology, trust, idle detection, scheduling, preemption, accounting, and integration.
- Execution engines remain plugins behind versioned job, capability, lifecycle, and result contracts.
- Installation of an AI desktop client or its user-scoped Session Host does not
  enroll the endpoint into the Compute Fabric.
- Managed campus workers require institution-authorized deployment, unique device
  identity, attestation, workload isolation, resource policy and remote revocation.
- Personal-device volunteer workers require a separate package or explicit
  component selection, informed opt-in, visible active/paused state, CPU/GPU,
  power/network/time limits and one-action withdrawal. They are disabled by
  default and cannot be made a condition of desktop-client access.
...
- [Node Enrollment and Attestation](Node-Enrollment-and-Attestation.md)
- [Idle and Interactive User Policy](Idle-and-Interactive-User-Policy.md)
